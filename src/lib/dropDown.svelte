...
<div class="sticky top-0 bg-white text-xl">
	<nav class="hidden lg:flex justify-between py-3 px-8">
		<div class="flex gap-20 w-1/2 items-center">
			<span><Logo /></span>
			<div>
				<button on:click={toggleFirstMenu} class="whitespace-nowrap"
					>Features<span>
						<Flechita />
					</span></button
				>
			...
			<div>
				<button class="whitespace-nowrap" on:click={toggleSecondMenu}
					>Company<span>
						<Flechita />
					</span></button
				>
	...
	<nav class="lg:hidden flex justify-between w-full items-center px-7 py-4">
		<div>
			<Logo />
		</div>
		<button on:click={toggleSidedMenu}>
			{#if toggleMenu}
				<Menu />
			{:else}
				<CloseMenu />
	...
	{#if toggleMenu}
		<div class="h-screen w-4/5 absolute right-0 bg-white pl-7 text-3xl flex flex-col gap-3">
